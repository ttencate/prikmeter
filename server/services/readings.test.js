...
describe('services/readings', () => {
  ...
  describe('create', () => {
    it('creates an electricity reading', async () => {
      ...
      await expect(readings.getForMeter({ id: reading.meterId, type: 'electricity' })).to.eventually.deep.equal([testDb.datesToTimestamps(reading)])
    })
    ...
    it('creates a gas reading', async () => {
      ...
      await readings.create(reading)
      await expect(readings.getForMeter({ id: reading.meterId, type: 'gas' })).to.eventually.deep.equal([testDb.datesToTimestamps(reading)])
    })
